...
class Shade:
	...
	def mineralLineTarget(self, townhall):
		...
		if self.game.units(MINERALFIELD).closer_than(15, townhall):
			mins = self.game.units(MINERALFIELD).closer_than(15, townhall)
			vasp = self.game.units(VESPENEGEYSER).closer_than(15, townhall)
			mf = Units((mins + vasp))
			f_distance = 0
			mineral_1 = None
			mineral_2 = None
