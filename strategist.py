...
####################
	...
	async def setupPositions(self):
		...
		if nexus:
			mins = self.game.units(MINERALFIELD).closer_than(15, nexus)
			vasp = self.game.units(VESPENEGEYSER).closer_than(15, nexus)
#			mf = Units((mins + vasp), self.game)
		...
		if self.ramp2:
			...
			if self.game.units(MINERALFIELD).closer_than(6, goto) or self.game.units(VESPENEGEYSER).closer_than(6, goto):
				#print ('moving pylon')
...
#######################
	...
	def save_expand(self):
		...
		self.train.allow_stalkers = True
