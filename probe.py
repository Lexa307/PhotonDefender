...
#####################
	...
	def LongDistanceMine(self):
		if len(self.unit.orders) == 0:
			for nexus in self.game.units(NEXUS).ready:
				...
				if self.game.units(MINERALFIELD).closer_than(100, nexus).exists:
					mf = self.game.units(MINERALFIELD).closer_than(100, nexus).closest_to(self.unit)
	...
	def OverMine(self):
		if len(self.unit.orders) == 0:
			for nexus in self.game.units(NEXUS).ready:
				#print (self.unit.tag, 'checking for minerals')
				if self.game.units(MINERALFIELD).closer_than(10, nexus).exists:
					mf = self.game.units(MINERALFIELD).closer_than(10, nexus).random
					self.game.combinedActions.append(self.unit.gather(mf, queue=False))
					return True
		else:
			if 'gather' in str(self.unit.orders).lower() or 'return' in str(self.unit.orders).lower():
	...
	def findMinerals(self):
		...
		for nexus in nexuslist:
			#print (self.unit.tag, 'checking for minerals')
			for mf in self.game.units(MINERALFIELD).closer_than(12, nexus).sorted(lambda x: self.unit.distance_to(nexus.position)):
	...
	def checkMinerals(self):
		if not self.vas_miner:
			...
			if self.game.units(MINERALFIELD).find_by_tag(self.gather_target.tag):
				return True
...
#################
	...
	def findRushDefMinerals(self):
		...
		for nexus in self.game.units(NEXUS).ready:
			#print (self.unit.tag, 'checking for minerals')
			for mf in self.game.units(MINERALFIELD).closer_than(10, nexus).sorted(lambda x: self.unit.distance_to(x.position)):
				return mf
		return None		
